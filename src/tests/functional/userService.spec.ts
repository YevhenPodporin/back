...
import {requestApp, logginTestUser} from '../index'
...
describe('Testing user service', () => {
    ...
    beforeAll(async () => {
        ...
        await seed()
        loggedUser = await logginTestUser()
    })

    it('Login and get profile', async () => {
        const res = await requestApp.get('/api/user/profile').auth(loggedUser.accessToken, {type: 'bearer'});
        ...
        expect(prisma.users.findFirst({where: {email: res.body.email}})).toBeTruthy()
    })
    ...
    it('Edit correct profile', async () => {
        ...
        const res = await requestApp.post('/api/user/profile/edit')
        ...
        const updatedUser = await prisma.profile.findFirst({where: {user_id: res.body.user_id}})
