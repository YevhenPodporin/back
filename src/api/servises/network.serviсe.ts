      ...
      if (isHasRequest) {
        if (status === RequestStatus.REJECTED) {
          ...
          return {
            errors: null,
            message:
              isHasRequest.status === RequestStatus.REQUEST
                ? "Request was dismissed successful"
                : "User removed from your friends successful",
          };
        } else if (
          status === RequestStatus.APPROVED &&
          isHasRequest.from_user_id === +to_user_id
        ) {
          ...
          isHasRequest.status === RequestStatus.REQUEST ||
          isHasRequest.status === RequestStatus.APPROVED
        ) {
        ...
        if (status === RequestStatus.REQUEST) {
          ...
          return {
            errors: `Incorrect status, should be:${RequestStatus.REQUEST}`,
          };
        }
      }
